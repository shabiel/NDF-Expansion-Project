PSN466D ;BIR/DMA-post install routine to load data ;2016-11-02  5:00 PM
 ;;4.0;NATIONAL DRUG FILE;**466**; 30 Oct 98
 ;
 ; VEN/SMH is annotating this routine! 
 ;
 ...
 ;
 ; (sam): TREC Is the total number of records; XUMF to override Master File Protection
 S PSNDF=1,XPDIDTOT=+$G(@XPDGREF@("TREC")),TREC=0
 S XUMF=1
 ;TO ALLOW ADDS TO 56 ,50.416,50.605,50.606, AND 50.6
 ;MORE ELEGANT CHANGE LATER
 ;
 ; (sam): DATANT contains the .01's of the records (are they all new? that's what it says here; but I don't think so.)
 S FILE=0,GROOT=$NA(@XPDGREF@("DATANT"))
 ;load new entries first
 ; (sam): FILE will be file number; GROOT1 will be XPD global ref to our data.
 ; (sam): JJ steps by 2 b/c entries in diads.
 ; (sam): DIA is the node that looks like "5024^3151208.11235^.01^358^A"
 ; (sam): NEW Is the data to file
 ...
 .S DA=+DIA K FDA,IENS
 .I $$GET1^DIQ(FILE,DA,.01)]"" S FDA(FILE,DA_",",.01)=NEW D FILE^DIE("","FDA") Q  ; (sam): Rename existing entry!
 .S DINUM=DA,X=NEW,DIC=ROOT,DIC(0)="L",DIC("DR")="S Y=0" K DD,DO D FILE^DICN  ; (sam): Adds new entries. But a STRANGE DIC("DR").
 .S TREC=TREC+1 I '(TREC#500) D UPDATE^XPDID(TREC)
 ;
 ; (sam): DATAN contains the .01 of the multiples
 ; (sam): Sample DIA: "15643,3997^3151208.095609^14,.01^358^A"
 ...
 .I FLDS["," D
 ..;it should, but
 ..S LI=$P(DIA,"^",3) F J=1:1:$L(LI,",")-1 S ROOT=+$P(^DD(ROOT,+$P(LI,",",J),0),"^",2) ; (sam): Get Subfile number
 ..S LI=$P(DIA,"^"),IENS="" F J=$L(LI,","):-1:1 S IENS=IENS_$P(LI,",",J)_"," ; (sam): Reverse top bottom DA's to get IENs (which is bottom up).
 ..S DA=+IENS ; (sam): DA is now the entry number in the subfile.
 .;I $$GET1^DIQ(ROOT,IENS,.01)]"" S FDA(ROOT,IENS,.01)=NEW D FILE^DIE("","FDA") Q
 .S FDA(ROOT,"+"_IENS,.01)=NEW,IEN(DA)=DA D UPDATE^DIE("","FDA","IEN") ; (sam): Add new subfile entry
 .S TREC=TREC+1 I '(TREC#500) D UPDATE^XPDID(TREC)
 .I FILE=50.68,$P(DIA,"^",3)="14,.01" S ^TMP("PSNN",$J,$P(DIA,"^"))="" ; (sam): If we changed the active ingredients, remember which one we added.
 ;                                                                     ; (sam): Format: ^TMP("PSNN",$J,"451,35") 451 = DA(1), 35 = DA
 ;
 ; (sam): DATAO is the rest of the data. Everything here is pretty much identical to the block above.
 ...
 .S TREC=TREC+1 I '(TREC#500) D UPDATE^XPDID(TREC)
 ;
 ; (sam): The index we kept above of the active ingredients; - now we put the data in 50.416 in a multiple
 S JJ=0 F  S JJ=$O(^TMP("PSNN",$J,JJ)) Q:'JJ  S DA=$P(JJ,",",2),DA(1)=+JJ D ING^PSNXREF
 ;
 ; (sam): I didn't see a "WORD" subscript before. So don't know what that is.
WORD S ROOT1=$NA(@XPDGREF@("WORD")),CT=0,ROOT2=$NA(@ROOT1@(0))
 ...
 ;
 ; (sam): Send two mail messages - both from data inside the global.
...
DRUGFILE ;
 ...
 ; (sam): I don't have "GENERIC" and "POE" in my sample.
 S ROOT1=$NA(@XPDGREF@("GENERIC")),ROOT2=$NA(@XPDGREF@("PRODUCT")),ROOT3=$NA(@XPDGREF@("POE")),DA=0
 ; (sam): Loop through every drug;
 ; (sam): NA = Name; CLA = Class; INV = Investigational? ; IN = Inactivation Date
 ; (sam): INA = Is the drug inactive? GE = VA Generic; PR = VA Product ; CMOP flag; VAPN = VA Product Name
 ...
 .S VAIN=$P($G(^PSNDF(50.68,PR,7)),"^",3) ; (sam): VA Product inactivation date.
 .; (sam): If Generic, Product in XPD global OR VAP Inactivated, then...
 .; (sam): put in ^TMP($J,Inactive/Investiational/Active,name^ien^inactivation date,1)= CMOP - VAPN - Inactivation Date
 ...
 ..; (sam): Delete ENTIRE ND node on the drug entry.
 ..S DIE="^PSDRUG(",DR="20////@;21////@;22////@;23////@;24////@;27////@;29////@;" D ^DIE K DIE,DR
 ..; (sam): Load doses into mail message; and then delete them.
 ..I PSN I $P($G(^PSDRUG(DA,"DOS")),"^")]""!$O(^("DOS1",0))!$O(^PSDRUG(DA,"DOS2",0)) D LOAD K ^PSDRUG(DA,"DOS"),^("DOS1"),^("DOS2")
 ..; (sam): Delete CMOP indicator; and say in the activity log that there is an NDF Update
 ..I $P($G(^PSDRUG(DA,3)),"^") S DIE=50,DR="213////0;" D ^DIE K DIE,DR I PSN1 S IND=$O(^PSDRUG(DA,4," "),-1),$P(^(IND,0),"^",6)="NDF Update"
 .;(sam): If "PRODUCT" node is there, kill doses (again!--this is not necessary) 
 .I PSN,$D(@ROOT3@(PR)) K ^PSDRUG(DA,"DOS"),^("DOS1"),^("DOS2")
 .;(sam): Now, just in case none of the unmatching happened, then...
 .;(sam): update the national formulary indicator and the VA Class text from PRIMARY VA DRUG CLASS field.
 .;(sam): now correct CMOP dispense
 ...
 .S TREC=TREC+1 I '(TREC#500) D UPDATE^XPDID(TREC)
 ;
 ; (sam) Send unmatched drugs message; and make it a priority
...
REINDEX ;Make sure APC xref is correct
 ; (sam): for each patient allergy, see if it has VA classes, and re-index them.
...
MORE ;REINDEXING
 ;now the APD
 ;(sam): We populated the 1 node from the VA PRODUCT file using ING^PSNXREF
 ;(sam): This updates the index.
